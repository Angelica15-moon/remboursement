...
function HistoriquePaiements() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) => {
        console.error(
          "Erreur lors de la récupération des données Excel :",
          error
        );
      });
  }, []);
  ...
  const generatePdf = (ref, list) => {
    ...
    const logoWidth = 50;
    const logoHeight = 20;
    doc.addImage(logoImage, "PNG", 20, 10, logoWidth, logoHeight);
    doc.text(title, 15, 40);
    doc.text("Date : " + formatDate(Date.now()), 130, 40);
    doc.text(clients.nom, 15, 46);
    doc.text("Id client : " + clients.id, 15, 52);
    doc.text("Reférence crédit : " + clients.RefCredit, 15, 58);
    doc.text("Montant Abandonnée : " + clients.MontantAbandonnee, 15, 64);
    doc.autoTable({
      startY: 72,
      styles: { lineWidth: 0.1, lineColor: "black" },
      columns,
      body: rows,
    });
